...
def admin_home_page():
    ...
    def df():
        ...
        return df
    def df_home():
        df_home = pd.read_csv("data/ulasan_tiket_kai_access_labeling_reviewer.csv")
        return df_home
    
    # @st.cache_data(show_spinner=False)
    # def df_2400():
    #     df_2400 = pd.read_excel("2400_pos_neg.xlsx")
    #     df_2400 = df_2400[['content', 'label']]
    #     return df_2400
...
#         pagination.dataframe(data=pages[current_page - 1], use_container_width=True)
    ...
    st.success("Sentiment Analysis Menggunakan Metode Text Mining " 
               "dan Random Forest Untuk Klasifikasi Keluhan Pengguna (Studi Kasus: Aplikasi KAI Access)",
               icon="ℹ️")

    st.divider()
    st.header("Dataset Ulasan KAI Access :")    
    with st.expander("6000 data hasil scraping", expanded=True):
        st.write("")
        st.dataframe(df(), use_container_width=True)
        
    with st.expander("6000 data hasil scraping yang telah diberi label oleh 3 orang reviewer", expanded=True):
        st.write("")
        st.dataframe(df_home(), use_container_width=True)
    
    # st.header("Filter Data Pengujian :")    
    # with st.expander("2400 data (1200 Positive dan 1200 Negative) untuk pengujian", expanded=True):
    #     st.write("")
    #     st.dataframe(df_2400(), use_container_width=True)
